...
"""
    ...
    print(f"Trying user configured model: {model_id}")
    
    try:
        # Claude models - use user's configured model ID
        ...
        error_str = str(e)
        print(f"User model failed: {error_str}")
        
        # Check for specific error types and provide helpful messages
        if "on-demand throughput isn't supported" in error_str:
            raise Exception(f"""
    ERROR: Your Claude Sonnet 4 model requires an inference profile, not direct model ID access.
    
    Please update your AWS_BEDROCK_MODEL_ID secret to use an inference profile ARN instead of the direct model ID.
    
    The format should be something like:
    arn:aws:bedrock:us-east-1:123456789012:inference-profile/your-profile-name
    
    Or use a cross-region inference profile like:
    us.anthropic.claude-3-5-sonnet-20240620-v1:0
    
    Contact your AWS administrator to set up the proper inference profile for Claude Sonnet 4.
    
    Original error: {error_str}
    """)
        elif "Input is too long" in error_str:
            raise Exception(f"""
    ERROR: The input context is too long for the model.
    
    This can happen when the codebase analysis generates too much content.
    Try reducing the task description or the codebase context size.
    
    Original error: {error_str}
    """)
        else:
            raise Exception(f"Model invocation failed: {error_str}")
